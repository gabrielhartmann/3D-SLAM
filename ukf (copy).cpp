...
{
//    Eigen::VectorXd cameraState;
//    cameraState = simCamera.getStateVector();
//    cameraState.conservativeResize(cameraStateSize);
    
    //    for (int i=0; i<numLandmarks; i++)
//    {
//        int index = getLandmarkIndex(i);
//        cameraState[index + 6] = cameraState[index + 6] + 0.5;
//    }
    
//    Eigen::VectorXd state;
//    state.resize(cameraStateSize);
//    clear(state);
//    state << 0.0, 100.0, 0.0,
//                     0.0, 0.0, 0.0;
//    print("State:", state);
...
//    
//    printf("ADD FEATURE\n\n\n");
//    for (int i=0; i<simCamera.map.size(); i++)
//    {
...
//        pixel = rotMat.transpose() * (simCamera.map[i].position() - position); // Landmark in camera coordinates
...
//        state[state.rows()-3] = pixel.x();
//        state[state.rows()-2] = pixel.y();
//        state[state.rows()-1] = simCamera.focalLength;
//        
...
//        covariance(covariance.rows()-1, covariance.rows()-1) = simCamera.measurementNoiseVariance.x(); //Actually focal length variance... but whatever
//        
...
//    stateCovariance = covariance;
    ...
    Eigen::VectorXd fakeState = simCamera.fakeInitialState;
    Eigen::VectorXd trueState = simCamera.trueInitialState;
    
    stateVector = fakeState;
    stateCovariance = (trueState - stateVector) * (trueState - stateVector).transpose();
...
void UKF::initializeProcessCovariance()
{
    ...
    processCovariance.resize(cameraNoiseSize + numLandmarks, cameraNoiseSize + numLandmarks); //landmarks only vary by inverse depth so only one entry per landmark
    //processCovariance.resize(noiseSize, noiseSize);
    ...
    for (int i=cameraNoiseSize; i<processCovariance.rows(); i++)
    {
...
double UKF::covarianceWeight(int index, double degree)
{
    if (index == 0)
    {
        return meanWeight(0, degree) + (1 - (alpha * alpha) + beta);
    }
...
//    print("Difference:", tmpDiff);
    ...
    for (int i=0; i<sigmaPoints.size(); i++)
    {
        ...
        Pxz = Pxz + covarianceWeight(i, sigmaPoints[i].rows()) * (tmpState * tmpMeasurement.transpose());
    }
    ...
    for (int i=0; i<sigmaPoints.size(); i++)
    {
        clear(tmpMeasurement);
        tmpMeasurement = predictedMeasurements[i] - aPrioriMeasurementsMean;
        Pzz = Pzz + covarianceWeight(i, sigmaPoints[i].rows()) * (tmpMeasurement * tmpMeasurement.transpose());
    }
...
void UKF::addLandmark(Eigen::VectorXd& sigmaPoint)
{
    ...
    Eigen::Vector3d direction;
    direction << sigmaPoint[sigmaPoint.rows()-3], sigmaPoint[sigmaPoint.rows()-2], sigmaPoint[sigmaPoint.rows()-1];
    ...
    double inverseDepth = simCamera.defaultInverseDepth;
    
    sigmaPoint.conservativeResize(sigmaPoint.rows() + 4);
